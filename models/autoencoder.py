...
class autoencoder(baseClassifier):
	...
	def buildModel(self):
		...
		reshaped = tf.reshape(self.X, [-1, self.FLAGS.num_features*self.FLAGS.time_length])

		bottleneck = tf.contrib.layers.fully_connected(reshaped, self.timelength, biases_initializer=tf.contrib.layers.xavier_initializer(),scope='step1/full1')
		bottleneck_min = tf.contrib.layers.fully_connected(bottleneck, 10, biases_initializer=tf.contrib.layers.xavier_initializer(),scope='step1/full2')
		bottleneck = tf.contrib.layers.fully_connected(bottleneck_min, self.timelength, biases_initializer=tf.contrib.layers.xavier_initializer(),scope='step1/full3')

		out = tf.contrib.layers.fully_connected(bottleneck, self.FLAGS.num_features*self.FLAGS.time_length, biases_initializer=tf.contrib.layers.xavier_initializer(),scope='step1/full4')

		outputs = tf.reshape(out, [-1, self.FLAGS.time_length, self.FLAGS.num_features])

		self.classification = tf.contrib.layers.fully_connected(bottleneck_min, num_outputs=self.num_classes, weights_initializer = tf.contrib.layers.xavier_initializer(), scope='step2')
		return outputs
